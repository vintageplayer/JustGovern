...
export default function Home() {
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 43113) {
      window.alert("Please switch to the Avax Fuji network!");
      throw new Error("Please switch to the Avax Fuji network");
    }
  ...
  // DAO Treasury Balance, User Voting Power, and Number of Proposals in the DAO
  useEffect(() => {
    if (!walletConnected) {
        ...
        <div>
          {proposals.map((p, index) => (
            <div key={index} className={styles.proposalCard}>
              ...
              <p>Proposal Titel: {p.proposalTitle}</p>
  ...
  return (
    <div>
      <Head>
        <title>Cross-Chain DAO</title>
        <meta name="description" content="Cross-Chain DAO" />
